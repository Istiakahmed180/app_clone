...
| Level 7 fixture imported through file picker | Base + ABI split selected as two files | Multi-select, staged APK set, manifest validation | FAIL after metadata fix: base install succeeded, standalone split install was rejected by Bcore | Not reached | Not reached | Not reached | FAIL | Not tested | Bcore imported-split install path: `installPackageAsUser(File)` parses the split as a standalone APK and returns `getPackageArchiveInfo` failure |
...
- The imported-APK path now carries an ordered APK set, retains all selected files per profile, validates package/version/base/split uniqueness, and reads standalone split manifest identity when Android's public archive API rejects that split. On the physical Debug run, this reached Bcore: base installation succeeded, then the standalone split failed with `getPackageArchiveInfo` parse failure. The Release host also builds successfully, but imported guest UI has not been reached.
