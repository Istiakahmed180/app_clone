...
| Markor 2.15.2 — Level 8C WebView | `net.gsantner.markor` | WebView preview / local storage | Single APK | PASS: actual user-facing import and install | PASS: guest UI and document activity reached | PARTIAL: embedded `android.webkit.WebView` rendered local preview; public URL opened Chrome Custom Tab, not embedded WebView | Not run after Release failure | PARTIAL | FAIL | PARTIAL — WebView compatibility incomplete | Release file-open path: Bcore guest application startup failed with PackageManager fallback/null APK paths and `ContextImpl`→`Application` ClassCastException |
