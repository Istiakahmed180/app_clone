...
| Markor 2.15.2 — Level 8C WebView | `net.gsantner.markor` | WebView preview / local storage | Single APK | PASS: actual user-facing import and install | PASS: guest UI reached in clean minified Release and after relaunch | PASS: local Markdown preview reached an embedded `android.webkit.WebView` in Debug and minified Release; no data-directory conflict after fix | PASS: same profile relaunched; persisted `webview_test.md` remained available | PASS | PASS | PASS — local WebView compatibility verified | Historical first fatal was `Using WebView from more than one process at once with the same data directory`; public URL still opens Chrome Custom Tab and is not counted as embedded WebView |
...
| VLC for Android 3.7.1 — Level 8D | `org.videolan.vlc` | Native media / storage / foreground playback | Single APK, arm64-v8a | PASS | PASS: MainActivity reached in Debug and Release | FAIL: media library remained Loading; Release logged thumbnail-directory permission denial; no playback verified | PARTIAL: same profile/MainActivity relaunched, playback still unverified | PARTIAL | PARTIAL | PARTIAL — next compatibility boundary identified | Android 15 storage-manager check and virtual media-library path permissions |
